...
class rseEnrolController extends dkFrameWork
{
    public function submitAction()
    {
		...
		$conf = array(
			...
			,"output_file_path"		=> "/workdir/srcs/output/cert/".$user_idx."/rseEnrol/".$cert_id."/rseEnrol.cert"
			,"cert_id"				=> array(
				 "type"		=> "hostname"
				,"value"	=> $cert_id
			)
		...
		// run
		chdir("/workdir/srcs");
		exec("rm -rf /workdir/srcs/output/cert/".$user_idx."/rseEnrol/".$cert_id);
		exec("mkdir -p /workdir/srcs/output/cert/".$user_idx."/rseEnrol/".$cert_id);
		exec("/workdir/srcs/cert_tool create-cert /var/www/scms2016gui/public/conf/rseEnrol.json 2>&1", $output);

		// compress
		exec("zip -jr /workdir/srcs/output/cert/".$user_idx."/rseEnrol/".$cert_id.".zip /workdir/srcs/output/cert/".$user_idx."/rseEnrol/".$cert_id.".cert");

		$result = array(
			 "result" => "success"
			,"download_path"	=> "/workdir/srcs/output/cert/".$user_idx."/rseEnrol/".$cert_id.".zip"
		);
