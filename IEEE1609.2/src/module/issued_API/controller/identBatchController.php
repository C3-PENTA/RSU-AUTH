...
class identBatchController extends dkFrameWork
{
    public function submitAction()
    {
		...
		$conf = array(
			...
			,"output_dir"			=> "/workdir/srcs/output/cert/".$user_idx."/ident/".$device_name."/".$cert_id
			,"obe_id"				=> $cert_id
		...
		// run
		chdir("/workdir/srcs");
		exec("rm -rf /workdir/srcs/output/cert/".$user_idx."/ident/".$device_name."/".$cert_id);
		exec("mkdir -p /workdir/srcs/output/cert/".$user_idx."/ident/".$device_name."/".$cert_id);
		exec("/workdir/srcs/cert_tool create_ident_batch /var/www/scms2016gui/public/conf/ident_batch.json 2>&1", $output);

		// compress
		exec("zip -jr -P ".$zip_password." /workdir/srcs/output/cert/".$user_idx."/ident/".$device_name."/".$cert_id.".zip /workdir/srcs/output/cert/".$user_idx."/ident/".$device_name."/".$cert_id);

		$result = array(
			 "result" => "success"
			,"download_path"	=> "/workdir/srcs/output/cert/".$user_idx."/ident/".$device_name."/".$cert_id.".zip"
		);
