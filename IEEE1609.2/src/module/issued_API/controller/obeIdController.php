...
class obeidController
{
    public function indexAction()
    {
		// 인증서 조회
		$dir_list = array();
		exec("ls /workdir/srcs/output/*/*/*/pca/*", $output);
		if(count($output)){
    ...
    public function submitAction()
    {
		...
		$conf = array(
			...
			,"output_file_path"		=> "/workdir/srcs/output/".$_POST["issuer_cert"]."/obeId/".$dirname."/obeId.cert"
			,"cert_id"				=> array(
		...
		// create conf file
		$fp = fopen("/var/www/scms2016gui/public/conf/obeId.json", "w");
		fwrite($fp, $conf);
		fclose($fp);

		// run
		chdir("/workdir/srcs");
		exec("mkdir -p /workdir/srcs/output/".$_POST["issuer_cert"]."/obeId/".$dirname);
		exec("python /workdir/srcs/cert_tool.py create-cert /var/www/scms2016gui/public/conf/obeId.json 2>&1", $output);
		...
		// compress
		exec("zip -jr /var/www/scms2016gui/public/downloads/obeId_".$dirname.".zip /workdir/srcs/output/".$_POST["issuer_cert"]."/obeId/".$dirname);

		$result = array(
			 "result" => "success"
			,"download_path"	=> "/downloads/obeId_".$dirname.".zip"
		);
