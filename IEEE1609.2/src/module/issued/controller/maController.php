...
class maController
{
    ...
    public function submitAction()
    {
		...
		$conf = array(
			...
			,"output_file_path"		=> "/workdir/srcs/output/".$_POST["issuer_cert"]."/ma/".$dirname."/ma.cert"
			,"cert_id"				=> array(
		...
		// create conf file
		$fp = fopen("/var/www/scms2016gui/public/conf/ma.json", "w");
		fwrite($fp, $conf);
		fclose($fp);

		// run
		chdir("/workdir/srcs");
		exec("mkdir -p /workdir/srcs/output/".$_POST["issuer_cert"]."/ma/".$dirname);
		exec("python /workdir/srcs/cert_tool.py create-cert /var/www/scms2016gui/public/conf/ma.json 2>&1", $output);
		...
		// compress
		exec("zip -jr /var/www/scms2016gui/public/downloads/ma_".$dirname.".zip /workdir/srcs/output/".$_POST["issuer_cert"]."/ma/".$dirname);

		$result = array(
			 "result" => "success"
			,"download_path"	=> "/downloads/ma_".$dirname.".zip"
		);
