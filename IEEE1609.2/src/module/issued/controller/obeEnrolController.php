...
class obeEnrolController
{
    public function indexAction()
    {
		// 인증서 조회
		$dir_list = array();
		exec("ls -R /workdir/srcs/output/*/*/eca/*/eca.cert", $output);
		if(count($output)){
    ...
    public function submitAction()
    {
		...
		$conf = array(
			...
			,"output_file_path"		=> "/workdir/srcs/output/".$_POST["issuer_cert"]."/obeEnrol/".$dirname."/".$_POST["obe_id"].".cert"
			,"cert_id"				=> array(
		...
		// run
		chdir("/workdir/srcs");
		exec("mkdir -p /workdir/srcs/output/".$_POST["issuer_cert"]."/obeEnrol/".$dirname);
		...
		// compress
		exec("zip -jr /var/www/scms2016gui/public/downloads/obeEnrol_".$dirname.".zip /workdir/srcs/output/".$_POST["issuer_cert"]."/obeEnrol/".$dirname);

		$result = array(
			 "result" => "success"
			,"download_path"	=> "/downloads/obeEnrol_".$dirname.".zip"
		);
