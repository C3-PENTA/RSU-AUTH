...
class laController
{
    public function indexAction()
    {
		// 인증서 조회
		$dir_list = array();
		exec("ls /workdir/srcs/output/*/*/*/eca/*", $output);
		if(count($output)){
    ...
    public function submitAction()
    {
		...
		$issuer_cert = "/workdir/srcs/output/".$_POST["issuer_cert"]."/eca.cert";

		$conf = array(
			 "cert_type"			=> "explicit"
			,"use_encryption_key"	=> $_POST["use_encryption_key"]
			,"issuer_cert_path"		=> $issuer_cert
			,"output_file_path"		=> "/workdir/srcs/output/".$_POST["issuer_cert"]."/la/".$dirname."/la.cert"
			,"cert_id"				=> array(
		...
		// create conf file
		$fp = fopen("/var/www/scms2016gui/public/conf/la.json", "w");
		fwrite($fp, $conf);
		fclose($fp);

		// run
		chdir("/workdir/srcs");
		exec("mkdir -p /workdir/srcs/output/".$_POST["issuer_cert"]."/la/".$dirname);
		exec("python /workdir/srcs/cert_tool.py create-cert /var/www/scms2016gui/public/conf/la.json 2>&1", $output);
		...
		// compress
		exec("zip -jr /var/www/scms2016gui/public/downloads/la_".$dirname.".zip /workdir/srcs/output/".$_POST["issuer_cert"]."/la/".$dirname);

		$result = array(
			 "result" => "success"
			,"download_path"	=> "/downloads/la_".$dirname.".zip"
		);
